...
<html>
    <head>
        ...
        <title>Add New Project Opening - MLDAG Charusat</title>
    </head>
    <body>
        <%
            ...
            if(session.getAttribute("addNewGrant")!=null){
                ...
                while(grants.next()){
                    if(grants.getString("grants").equals("addProjectOpening")){
                        isGrant = false;
                        String title = request.getParameter("title");
                        String file = request.getParameter("file");
                        if(title.trim().length()>0&&file.trim().length()>0){
                            ps = conn.prepareStatement("insert into projectopening(title,file) " + "values(?,?)");
                    ...
                    if(isGrant){
            %>
                        <h3 style="color: red">You do not have the grants to add new project opening.Contact Admin to provide you this grant</h3>
